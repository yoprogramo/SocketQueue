...
function toHexString(byteArray,max) {
  var res =  Array.from(byteArray, function(byte) {
    return ('0' + (byte & 0xFF).toString(16)).slice(-2);
  }).join('');
  if (res.length>max){
    res = res.slice(0,max-1);
  }
  return res;
}
...
var ISO8583 = function(packagerName) {
  ...
  this._unpack = function(msg, id) {
    var result;
    console.log('['+id+'] Decoding: '+toHexString(msg,40)+"...");
    try {
  ...
  this.getBinMask = function(data) {
    ...
    var dummy = new Buffer([0, 0, 0, 0, 0, 0, 0, 0], 'ascii');
    var maskBuf = new Buffer(8);
    dummy.copy(maskBuf);
